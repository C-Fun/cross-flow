...
def broadcast_parameters(module, src=0):
    """Broadcast module parameters and buffers from `src` rank to all ranks.

    Used at the start of training so every rank starts from identical weights
    (we do manual gradient all-reduce rather than wrapping in DDP, because
    torch.func.jvp does not compose with DistributedDataParallel).
    """
    if process_count() == 1:
        return
    for tensor in list(module.parameters()) + list(module.buffers()):
        torch.distributed.broadcast(tensor.data, src=src)


def all_reduce_gradients(module):
    """Average gradients across all ranks in-place (manual DDP)."""
